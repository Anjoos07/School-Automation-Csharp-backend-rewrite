...
public static class FormOperations
{
    public static void MapFormOperationsEndpoints(this WebApplication app){
        ...
        {
            return Results.Ok(await FormModel.GetModel($"https://api.tally.so/forms/{formId}"));
        });
        app.MapGet("/form", async () =>
        {
            return Results.Ok(await FormModel.GetModel($"https://api.tally.so/forms"));
        });
        app.MapGet("/form-question/{formId}", async (string formId) =>
        {
            return Results.Ok(await FormModel.GetModel($"https://api.tally.so/forms/{formId}/questions"));
        });
        app.MapGet("/form-delete-submission/{formId}/{submissionId}",async (string formId, string submissionId) =>
        {
            return Results.Ok(await FormModel.GetModel($"https://api.tally.so/forms/{formId}/submissions/{submissionId}"));
        });
